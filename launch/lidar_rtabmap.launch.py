#recalibrated the original launch file to work with a stereo camera and RTAB-Map
import os
...
def launch_setup(context, *args, **kwargs):      
    ...
    return [
        DeclareLaunchArgument('stereo', default_value='false', description='Disable stereo'),
        DeclareLaunchArgument('depth', default_value='false', description='Disable depth'),        DeclareLaunchArgument('subscribe_rgb', default_value=LaunchConfiguration('depth'), description=''),
...
def generate_launch_description():
    ...
    return LaunchDescription([
        ...
        DeclareLaunchArgument('frame_id',       default_value='base_footprint',          description='Fixed frame id of the robot (base frame), you may set "base_link" or "base_footprint" if they are published. For camera-only config, this could be "camera_link".'),  #base_link changed to base_footprint for stability testing
        ...
        DeclareLaunchArgument('database_path',  default_value='~/.ros/lidar_rtabmap.db',  description='Where is the map saved/loaded.'),
        ...
        DeclareLaunchArgument('camera_info_topic', default_value='/camera/camera_info', description=''),

        
        # Stereo related topics
        DeclareLaunchArgument('stereo_namespace',        default_value='/stereo', description=''),
        DeclareLaunchArgument('left_image_topic',        default_value=[LaunchConfiguration('stereo_namespace'), '/left/image_raw'], description=''),
        DeclareLaunchArgument('right_image_topic',       default_value=[LaunchConfiguration('stereo_namespace'), '/right/image_raw'], description='Use grayscale image for efficiency'),
        ...
        DeclareLaunchArgument('odom_topic',                 default_value='/rtabmap/odom',  description='Odometry topic name.'),    # Using /rtabmap/odom instead of /odom for improved visual performance (empirical observation)
